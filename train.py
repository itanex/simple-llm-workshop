...
def load_data(filepath, block_size, batch_size, device):
    ...
    def get_batch(split_tokens):
        ...
        y = torch.stack([split_tokens[i + 1 : i + block_size + 1] for i in ix]).to(
            device
        )
...
def train(
    ...
    (
        ...
        stoi,
        itos,
    ) = load_data(data_path, block_size, batch_size, device)

    config = GPTConfig(vocab_size, block_size, n_layer, n_head, n_embed)
    model = GPT(config).to(device)
    print(
        f"Model: {n_layer}L/{n_head}H/{n_embed}D, "
